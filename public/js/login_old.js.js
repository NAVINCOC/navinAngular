...
function emailValidate(name) {
    ...
    //getData('/data', headers, success, error);
      ...
      postData('/verifyEmail', headers, data, function(data) {
        ...
        if(name=='login') {
        	if(data === 'NO') {
				$('#loginForm #errorEmail').text("Please register with this email");
				check="NO";
			}
			if(data === 'YES') {
		...
		if(name=='register'){
			if(data === 'YES') {
				$('#registerForm #errorEmailR').text("Email-id already exist");
				check='YES';
			}
			if(data === 'NO') {
				//$('#successEmail').text("Email verified");
  ...
  function validateRegidter(){
    ...
    $('#errorPassword').html('');
    $('#loginPassword').val();
    if(emailR=="" || passwordR=="" || confirmPasswordR=="" || address1=="" || address1=="" || address2=="" || address3=="" || country=="" || state=="" || city=="" || pincode=="" || contactno=="" || contactperson=="" || company=="" || industryType=="") {
